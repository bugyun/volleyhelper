...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    public void setJsonConvertFactory(JsonConvertFactory jsonConvertFactory) {
    ...
    public String getUrl() {
        switch (mMethod) {
            case Method.GET:
                mUrl = encodeGetParameterUrl(mUrl, paramsMap);
                break;
        }
        return mUrl;
    }
     ...
     * TODO 设置头信息
     */
    public void setHeaders(String headParams, String params) {
        if (headersMap == null) {
            headersMap = new ArrayMap<>();
        }
        headersMap.put(headParams, params);
    }
    ...
    public Map<String, String> getHeaders() throws AuthFailureError {
        ...
        return Collections.emptyMap();// 创建一个空的Map,如果这个map被置为不可用，可以通过以下实现
    }
    ...
    /**
     * TODO 增加添加参数的方法
     * Creates a param for post request.
     *
     * @param param
     * @param value
     */
    ...
    /**
     * TODO  增加一个get方法在setLoadParams方法中设置url
     * 用来拼接get请求的url地址
     *
     * @param url    请求的链接
     * @param params 各种参数
     * @return 拼接完成的链接
     */
    private String encodeGetParameterUrl(String url, Map<String, String> params) {
        if (params != null && params.size() > 0) {
            Uri uri = Uri.parse(url);
